<template>
	...
	<view class="">
		...
		<u-navbar bgColor="transparent">
			<view slot="left">
				<i class="ri-arrow-left-s-line text-white text-4xl"
		...
		<view class="px-4 py-2 bg-white" style="border-top-left-radius: 20rpx; border-top-right-radius: 20rpx;">
			...
			<view class="text-base leading-none text-gray-500 mt-2">角色名望：{{ user.total_mw || 0 }}</view>
			<!-- Math.floor(Math.random()*10) -->
			<view class="text-base leading-none text-gray-500 mt-2">雅集号：{{ user.uid|| '********' }}</view>
			<view class="text-base leading-none text-gray-500 mt-2">
				IP属地：{{ user.province || '未知' }}
			</view>
			<!-- <view class="mt-4">{{ user.bio || '暂无介绍' }}</view> -->
			<view class="flex mt-4">
				<block v-for="(tag, index) in user.tags" :key="index" :item="tag">
					<view
						class="border border-solid border-gray-200 p-2 rounded-full text-base leading-none text-gray-500 mr-2">
						{{ tag }}
					</view>
				</block>
			</view>
			<view class="!border-t border-0 border-solid border-gray-200 mt-6 mb-6"></view>
			<u-tabs :list="tablist" lineColor="rgba(255, 0, 0, 0.2)" lineWidth="70rpx" lineHeight="16rpx"
