<template>
	<view style="background: #f7f7f7;min-height: 100vh;">
		...
		<view @click="openAddPost"
			...
			<image style="width: 306rpx;height: 85rpx;" src="@/static/canyuyaling.png" mode="widthFix"></image>
		</view>
		<view>

			<feiWaterfall :lists="list" :from="'topic'" @waterfallClick="waterfallClick"></feiWaterfall>
			<view v-if="!list.length" style="text-align: center;margin-top: 50rpx;">
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.yalingId = this.$Route.query.yalingId
			this.setFontFamily()
			this.getLists()
			this.topicspeedList = [];
			uni.$on("addPostOk", () => {
		...
		methods: {
			...
			openAddPost() {
				this.$u.route('/pages/post/add', {
					yaling: this.title,
					yalingId: this.yalingId
				})
				// let that = this;
				// var obj = {
				// 	title: that.title,
