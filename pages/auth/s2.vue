<template>
    ...
    <view class="px-4">
		...
		<view style="padding: 200rpx 50rpx 20rpx 50rpx;" v-show="noRole_Count">
		    <view class="flex text-left text-2xl text-white">选择朝代</view>
			...
			<view class="" style="margin-top: -40rpx;margin-left: 200rpx; font-size: 28rpx;color: #999;line-height: 40px;">暂无角色可用</view>
				<view class="font1">非常抱歉！角色库中所有角色都被占用，我们正在加紧补充角色。您可以先以无名氏身份进入，稍后在个人中心获取角色。</view>
			
		</view>
		
		
        <view style="padding: 200rpx 50rpx 20rpx 50rpx;" v-show="isRole_Count">
            <view class="flex text-left text-2xl text-white">选择朝代</view>
            <view class="flex text-left mt-4">
                <text class="text-white opacity-50">选择穿越到的朝代，以便给你创建角色。</text>
            </view>
            <view class="flex flex-col mt-4" style="height: 600rpx; overflow-y: scroll;">
              <!-- <view class="flex items-center p-2 rounded-lg mt-4" v-for="(item, index) in listRoleDynasty" :key="index" :item="item" :class="dynasty.id === item.id ? 'bg-white text-rose-500' : 'text-white'" @click="handleChangeDynasty(item)">	
				...
				<view class="flex items-center p-2 rounded-lg mt-4" v-for="(item, index) in listRoleDynasty" :key="index" :item="item" :class="dynasty.id === item.id ? 'bg-white text-rose-500' : 'text-white'" @click="handleChangeDynasty(item)">
					<view v-show="isWomen==2" class="" style="width: 1000rpx ;" :class="[item.role_woman_count==0?'activedis':'']" >	
							<text class="flex-1">{{ item.dynasty }}</text>
							<text class="text-base opacity-50" style="float: right;">{{ $tools.erayear(item.startyear) + ' - ' + $tools.erayear(item.endyear) }}</text>
					</view>
					<view  v-show="isMan==1" class="" style="width: 1000rpx ;" :class="[item.role_man_count==0?'activedis':'']" >
							<text class="flex-1">{{ item.dynasty }}</text>
							<text class="text-base opacity-50" style="float: right;">{{ $tools.erayear(item.startyear) + ' - ' + $tools.erayear(item.endyear) }}</text>
					</view>
				</view>
        ...
        <u-modal :show="showRole" :showConfirmButton="false" :showCancelButton="false">
            <view class="w-full text-center">
                ...
                <view class="grid grid-cols-2 gap-4 mt-10 text-center">
                    ...
                    <view class="rounded-full p-4 text-base leading-none text-white bg-gradient-to-r from-rose-400 to-rose-500" style="background-color: #999; width: 490rpx;margin-top: -45rpx;":disabled="!times" @click="handleSubmit()"  >开始体验</view>
				...
				<view v-show="isNo"   class="rounded-full p-4 text-base leading-none bg-gray-100" style="width: 490rpx;margin-left: 0rpx;margin-top: 10rpx;" @click="handleRematch()">重新匹配(免费{{ times }}次)</view>
				</view>
            </view>
        </u-modal>
    </view>
...
export default {
    ...
    methods: {
        ...
        skip() {
			...
			let gender=uni.getStorageSync('gender')
			that.formGender=gender
			console.log(that.formGender,'ss');
			//记得清除
			let data={
				gender:that.formGender
			}
