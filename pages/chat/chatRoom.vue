<template>
	<view class="chatRoom">
		...
		<view class="roomBody">
			<view class="roomHead">
				<view style="display: flex;align-items: center;">
					...
					<view class="headLeft">
						<text style="color: #333333;font-size: 26rpx;">{{roomData.room_name||'未知'}}</text>
					</view>
				</view>
			...
			<view class="stageContainer">
				<!-- 未准备倒计时 -->
				<view class="markContainer" v-if="readyTimeFlag">
					<view class="markBox">
						<block v-for="(item,index) in roomPosition.stage_list" :key="index">
							<view class="markStatus" :style="{opacity:item.ready_status==1?'0':'1'}">
								...
								<text style="margin-left: 10rpx;">{{times}}s</text>
							</view>
						</block>
					</view>
				</view>
				<view class="markContainer" v-if="competitionFlag">
					<view class="markBox" v-if="taskData!=null">
						<view class="markStatus">
							<block v-if="taskData.answer_uid==roomPosition.stage_list[0].user_id">
								<i class="iconfontcolor icon-shaloudaojishi" style="font-size: 23rpx;"></i>
								<text style="margin-left: 10rpx;">{{times}}s</text>
							</block>
							<block v-else>
						...
						<view class="markStatus">
							<block v-if="taskData.answer_uid==roomPosition.stage_list[1].user_id">
								<i class="iconfontcolor icon-shaloudaojishi" style="font-size: 23rpx;"></i>
								<text style="margin-left: 10rpx;">{{times}}s</text>
							</block>
							<block v-else>
				...
				<view class="stageBox">
					<view class="positionNumBox" @click="openMai(roomPosition.stage_list[0],1)"
						:style='{background:roomPosition.stage_list[0].mike_status?"#FFA000":""}'
						style="margin-right: 10rpx;">
						<block v-if="roomPosition.stage_list[0].mike_status==0||roomPosition.stage_list[0]==''">
						...
						<block v-else>
							<block v-if="waveData!=null">
								<block v-for="(item,index) in waveData" :key="index">
									<block v-if="Number(item.userId)==roomPosition.stage_list[0].user_id">
					...
					<view class="positionBox">
						<block v-if="roomPosition.stage_list[0]">
							<image style="width: 125rpx;height: 170rpx" :src="roomPosition.stage_list[0].profile_url"
								mode="aspectFill">
							</image>
							<view class="userName">
								<text
									style="position: relative;z-index: 2;">{{roomPosition.stage_list[0].role_realname}}·{{roomPosition.stage_list[0].role_dynasty}}</text>
								<view class="line" v-if="roomPosition.stage_list[0].is_challenger"></view>
							</view>
							<view style="height: 32rpx;">
								<block v-if="competitionFlag==false">
									<block v-if="readyFlag!=true">
										<view class="stageStatus" @click="ready(roomPosition.stage_list[0])"
											v-if="roomPosition.stage_list[0].ready_status==0">准备
										</view>
										<view class="stageStatus" v-else>已准备</view>
									</block>
								</block>
								<block v-else>
									<view class="stageStatus"
										v-if="taskData.answer_uid==roomPosition.stage_list[0].user_id">思考中
									</view>
						...
						<block v-else>
							<image src="@/static/iconImage/zuo.png" @click="addStage(1)"
								style="width: 108rpx;height: 108rpx;" mode=""></image>
							<text class="empty" @click="addStage(1)">虚位以待</text>
						</block>
					</view>
				</view>
				<view class="stageCenter">
					<block v-if="keyWordData.content!=undefined&&keyWordData.content!=''&&keyWordData.length!=0">
						<view class="shanziBox">
							<image src="../../static/iconImage/shanzi.png" style="width: 215rpx;height: 215rpx;"
								mode="widthFix">
							</image>
							<text class="stageText">{{keyWordData.content}}</text>
						</view>
						<view class="stageCenterTextBox">
							<block v-for="(item,index) in taskContentList" :key="index">
								<block v-if="item.last_content!=''">
									<view class="stageCenterUserOne"
										v-if="item.answer_uid!=Number(roomPosition.stage_list[0].user_id)">
										<image src="../../static/leftjiao.png"
											style="width: 15rpx;height: 15rpx;position: absolute;top: -1rpx;left: -1px;"
											mode=""></image>
										<view>{{item.last_content}}</view>
									</view>
									<view style="display: flex;justify-content: flex-end;" v-else>
										<view class="stageCenterUserTwo">
											<image src="../../static/rightjiao.png"
												style="width: 15rpx;height: 15rpx;position: absolute;top: -1rpx;right: -1px;"
												mode=""></image>
											<view>{{item.last_content}}</view>
										</view>
									</view>
				...
				<view class="stageBox" style="justify-content: end;">
					<view class="positionBox">
						<block v-if="roomPosition.stage_list[1]">
							<image style="width: 125rpx;height: 170rpx;" :src="roomPosition.stage_list[1].profile_url"
								mode="aspectFill">
							</image>
							<view class="userName">
								<text
									style="position: relative;z-index: 2;">{{roomPosition.stage_list[1].role_realname}}·{{roomPosition.stage_list[1].role_dynasty}}</text>
								<view class="line" v-if="roomPosition.stage_list[1].is_challenger==1"></view>
							</view>
							<view style="height: 32rpx;">
								<block v-if="competitionFlag==false">
									<block v-if="readyFlag!=true">
										<view class="stageStatus" @click="ready(roomPosition.stage_list[1])"
											v-if="roomPosition.stage_list[1].ready_status==0">准备
										</view>
										<view class="stageStatus" v-else>已准备</view>
									</block>
								</block>
								<block v-else>
									<view class="stageStatus"
										v-if="taskData.answer_uid==roomPosition.stage_list[1].user_id">思考中
									</view>
									<view class="stageStatus" v-else>等待中</view>
								</block>
							</view>
						...
						<block v-else>
							<image src="@/static/iconImage/zuo.png" @click="addStage(2)"
								style="width: 108rpx;height: 108rpx;" mode=""></image>
							<text class="empty" @click="addStage(2)">虚位以待</text>
						</block>
					</view>
					<view class="positionNumBox" @click="openMai(roomPosition.stage_list[1],2)"
						:style='{background:roomPosition.stage_list[1].mike_status?"#FFA000":""}'
						style="margin-left: 10rpx;">
						<block v-if="roomPosition.stage_list[1].mike_status==0||roomPosition.stage_list[1]==''">
						...
						<block v-else>
							<block v-if="waveData!=null">
								<block v-for="(item,index) in waveData" :key="index">
									<block v-if="Number(item.userId)==roomPosition.stage_list[1].user_id">
										<wave :nums="item.volume"></wave>
									</block>
								</block>
							</block>
			...
			<view class="contentContainer">
				<view class="contentLeft">
					<scroll-view class="chatContainer" scroll-y="true" :scroll-into-view="scrollInto"
						...
						<block v-for="(item,index) in messageList.list" :key="index">
							<!-- 聊天消息 -->
				...
				<view class="contentRight">
					<view class="maiUser" v-for="(item,index) in roomPosition.wait_list" :key="index">
						<block v-if="item!=''">
							<image @click="openUserBox(item)" style="width: 100rpx;height: 100rpx;border-radius: 50%;"
						...
						<block v-else>
							<image src="@/static/iconImage/zuo.png" style="width: 100rpx;height: 100rpx;" mode="">
							</image>
					...
					<block v-for="(item,index) in roomPosition.stage_list" :key="index">
						<view v-if="item.user_id==userInfo.id" @click="useSilk" style="text-align: center;">
							<view
								style="padding: 0rpx 20rpx 0 0;margin-top: 0rpx;display: flex;align-items: center;;height: 57rpx;background: linear-gradient(135deg, #FFF8B6, #fff);margin-right: -30rpx;border-radius: 50rpx 0rpx 0rpx 50rpx;border: 1px solid #fff;">
								<image src="../../static/iconImage/libao.gif"
									style="width: 80rpx;height: 80rpx;margin-left: -20rpx;" mode="">
								</image>
								<view style="display: flex;align-items: center;">
									<image v-if="silkText!='免费'" src="../../static/qian.png"
										style="width: 35rpx;height: 35rpx;margin-right: 5rpx;" mode=""></image>
									<text
										style="color:#FFA000 ;font-size: 26rpx;font-family: font-test !important;white-space: nowrap;">{{silkText}}</text>
								</view>
					...
					<view @click="clickPractice" style="text-align: center;margin-top: 35rpx;">
						<view
							style="padding: 0rpx 20rpx 0 0;margin-top: 0rpx;display: flex;align-items: center;;height: 57rpx;background: linear-gradient(135deg, #FFF8B6, #fff);margin-right: -30rpx;border-radius: 50rpx 0rpx 0rpx 50rpx;border: 1px solid #fff;">
							<image src="../../static/lianxishu.png"
								style="width: 80rpx;height: 80rpx;margin-left: -20rpx;" mode="aspectFill">
							</image>

							<text
								style="color:#FFA000 ;font-size: 26rpx;font-family: font-test !important;white-space: nowrap;padding-left: 10rpx;">练习</text>
						</view>
					</view>
					<!-- <view>
			...
			<view class="bottomBar">
				<view class="tishi" :style="{visibility:tishiFlag?'visible':'hidden'}">
					<view style="display: flex;align-items: center;padding: 0rpx 20rpx;">
						<block v-if="roomType=='fhl'">
							<view style="color: #000000;font-size: 28rpx;height: 68rpx;line-height: 60rpx;">
								请发一句含【{{keyWordData.content}}】字的诗</view>
						</block>
				...
				<view style="display: flex;align-items: center;">
					...
					<block>
						<view v-if="positionFlag==false" class="maiBtn" @click="addTalk">
							<i class="iconfont icon-huatong-F"
			...
			<view class="drawerContainer">
				<view>
					...
					<view class="drawerMenu">
						...
						<block v-if="roomData.length!=0">
							<view class="menuItem" v-if="roomData.permission.close_room!=0" @click="closeRoom">
								<view class="iconMenu">
						...
						<!-- 	<view class="menuItem" v-if="roomData!=null&&roomData.room_info.public_type!='公共房'">
											<view class="iconMenu">
					...
					<view style="display: flex;align-items: center;justify-content: space-between;">
						...
						<view class="drawerOperate">
							<view class="drawerOperateText"
								v-if="roomData.length!=0&&roomData.permission.remove_user!=0" @click="drawerTabIndex=1"
								:style="{color:drawerTabIndex==1?'#fff':'#BDBDBD'}" style="margin-right: 10rpx;">
						...
						<view class="userItem">
							...
							<view style="color: #fff;font-size: 28rpx;" v-if="item.is_room_owner==0">
								...
								<text v-if="drawerTabIndex==2 && item.user_id!=userInfo.id"
									@click="$u.route('/pages/public/report',{user_id:item.user_id,type:'语音房',selectId:roomData.room_id})">举报</text>
							</view>
						</view>
					</block>
				</scroll-view>
		...
		<!-- 结果 -->
		<u-popup :show="taskResultShow" @close="taskResultShow = false;" mode="center" :closeable="false" :round="20"
			:customStyle="{background:'transparent'}">
			<view class="tokenContainer">
				<block v-if="taskResultData!=null">
					...
					<view @click="taskResultShow = false;"
		...
		<!-- 令牌 -->
		<u-popup :show="keyWordShow" @close="keyWordShow = false;" mode="center" :closeable="false" :round="20"
			:customStyle="{background:'transparent'}">
			<view class="tokenContainer">
				<block v-if="keyWordList.length!=0">
					<view class="tokenTitle">
						<text class="text">选择令牌</text>
						<view class="replace" @click="changeKeyWord">
							<text>换一批</text>
					...
					<view class="tokenTextList">
						<block v-for="(item,index) in keyWordList" :key="index">
							<view class="tokenTextItem" @click="selectKeyWordClick(item,index)"
								:style="{background:keyWordIndex==index?'#FFA000':'',color:keyWordIndex==index?'#FFFFFF':''}">
								{{item.word}}
							</view>
						</block>
					</view>
					<view class="tokenBens">
						<view class="tokenBen" style="margin-right: 15rpx;" @click="keyWordShow=false;keyWordIndex=0">
							取消</view>
						<view class="tokenBen" style="margin-left: 15rpx;" @click="keyWoridOk">确定</view>
					</view>
				</block>
			</view>
		</u-popup>
		...
		<u-popup :show="showCreateRoom" @close="showCreateRoom = false" mode="bottom" :closeable="false" :round="20"
			...
			<view class="createRoomContainer">
				...
				<view class="roomType" style="display: flex;align-items: center;margin-top: 85rpx;"
					...
					<view style="display: flex;align-items: center;">
						<block v-for="(item,index) in createroomData.type" :key="index">
				...
				<view class="roomType" style="display: flex;align-items: center;margin-top: 85rpx;">
					...
					<view style="display: flex;align-items: center;">
						<block v-for="(item,index) in createroomData.public_type" :key="index">
							<view style="display: flex;align-items: center;margin-left: 35rpx;"
				...
				<view class="createBtn" @click="createRoom">
					<block v-if="createRoomIndex==0">
						<text v-if="createroomData.need_pay ==1">花费{{createroomData.room_price}}铜钱创建</text>
						<text v-if="createroomData.need_pay ==0">免费创建</text>
					</block>
					<block v-else>
						<text>免费创建</text>
					</block>
		...
		<!-- 认输 -->
		<u-popup :show="exitRoomFlag " @close="exitRoomFlag  = false" mode="center" :closeable="false" :round="20"
			:customStyle="{background:'transparent'}">
			<view class="report">
				...
				<view
					...
					<view class="reportBtn" @click="exitRoomFlag  = false">继续比赛</view>
				</view>
			</view>
		</u-popup>
		<!-- 动画 -->
		<collision ref="collision">
			<template slot="left">
				<view style="display: flex;align-items: center;">
					<image :src="roomPosition.stage_list[0].profile_url"
						style="width: 125rpx;height: 170rpx;margin-bottom: 4rpx;" mode="aspectFill"></image>
					<view style="width: 132rpx;color: #000;font-size: 22rpx;margin-top: 30rpx;">
						{{roomPosition.stage_list[0].role_realname}}·{{roomPosition.stage_list[0].role_dynasty}}
					</view>
				</view>
			</template>
			<template slot="right">
				<view style="display: flex;align-items: center;justify-content: flex-end;">
					<text
						style="width: 132rpx;color: #000;font-size: 22rpx;text-align: left;margin-top:30rpx;">{{roomPosition.stage_list[1].role_realname}}·{{roomPosition.stage_list[1].role_dynasty}}</text>
					<image :src="roomPosition.stage_list[1].profile_url"
						style="width: 125rpx;height: 170rpx;margin-bottom: 4rpx;" mode="aspectFill"></image>
				</view>
			</template>
		</collision>
		...
		<u-popup :show="showGift" @close="showGift = false" mode="bottom" :closeable="false" :round="20"
			...
			<view class="giftContainer">
				<scroll-view scroll-y="true" class="giftBox">
					<block>
						<view v-if="roomPosition.stage_list[0]!=''||roomPosition.stage_list[1]!=''"
							style='display: flex;align-items: center;justify-content: space-between;padding-right: 30rpx;'>
							<text>送给：</text>
							<view v-if="roomPosition.stage_list.includes('')==false"
								style='display: flex;align-items: center;' @click="allGiftSelect">
	...
	export default {
		...
		data() {
			return {
				...
				keyboardHeight: 0,
				taskResultData: null,
				taskResultShow: false,
				taskData: [],
				tishiFlag: false,
				exitRoomFlag: false,
				taskVersion: null,
				clickXiamaiFlag: false,
				silkText: "免费",
				inviteList: [],
				showShare: false,
		...
		onLoad(e) {
			...
			uni.$on("changeRoom", (data) => {
				if (data.msg == 'chatRoom') {
					// getApp().globalData.socketTask._callbacks.message.splice(1);
					var storageRoomData = uni.getStorageSync("roomData");
					var type = '';
					switch (storageRoomData.room_info.room_type) {
						case "飞花令":
							type = 'fhl'
							break;
						case "成语接龙":
					...
					uni.redirectTo({
						url: `/pages/chat/chatRoom?roomId=${storageRoomData.room_info.room_id}&type=${type}`,
		...
		methods: {
				...
				try {
					this.$api("room.entryRoom", {
						...
						if (res.code == 1) {
							...
							this.initTRTC()
							// #endif
							this.getRoomPositionInfo()
							this.getRoomImInfo()
							this.getRoomTaskInfo()
							this.initSocket()
						} else {
							uni.showToast({
								icon: "none",
			...
			getRoomPositionInfo() {
				try {
					this.$api("room.getRoomPositionInfo", {
						room_id: this.roomId,
						index: ""
					}).then(res => {
						if (res.code == 1) {
							this.roomPosition.stage_list = res.data.stage_list;
							this.roomPosition.wait_list = res.data.wait_list;
							this.roomPosition.version = res.data.version;
							this.positionAllUserList = [...res.data.stage_list, ...res.data.wait_list];
							res.data.stage_list.forEach((item, index) => {
								if (item.countdown != undefined) {
									if (item.countdown != 0) {
										clearInterval(this.readyInterVal)
										clearInterval(this.interVal)
										this.readyTimeFlag = true;
										this.times = item.countdown;
										this.roomInterVal()
										return
									}
								}
							})
							this.positionAllUserList.forEach((item, index) => {
								if (item.mike_status != undefined) {
									if (item.mike_status == 1) {
										// #ifdef APP
										console.log(this.userId)
										console.log(this.roomId)
										let timeOut = setTimeout(() => {
											this.addAudioRoom(this.userId, this.roomId)
										}, 1000)
										// #endif
									}
								}
			...
			getRoomImInfo() {
				try {
					this.$api("room.getRoomImInfo", {
						room_id: this.roomId
					}).then(res => {
						if (res.code == 1) {
							this.messageList.list = [];
							this.messageList.version = res.data.version;
							res.data.list.reverse().forEach((item, index) => {
								this.messageList.list.push(item);
								this.scrollBottom();
							})
						}
					})
				} catch (err) {
					console.log("获取用户聊天失败")
				}
			},
			//任务信息
			getRoomTaskInfo() {
				try {
					this.$api("room.getRoomTaskInfo", {
						room_id: this.roomId
					}).then(res => {
						console.log(res)
						if (res.code == 1) {
							this.taskVersion = res.data.version
							if (res.data.answer_uid != undefined) {
								this.competitionFlag = true;
								this.keyWordData = res.data.content;
								this.taskData = res.data;
								this.taskContentList = [];
								this.taskContentList.push(res.data)
							}
						} else {
							console.log("获取任务失败")
						}
			...
			initSocket() {
				...
				getApp().globalData.socketTask.onMessage((res) => {
					...
					if (socketDate.cate == 'room') {
						...
						switch (socketDate.data.type) {
							case "entry_room":
								...
								that.messageList.list.push(item);
								that.scrollBottom();
								break;
							case "profile_change_one":
								that.roomPosition.stage_list = socketDate.data.stage_list;
								that.roomPosition.wait_list = socketDate.data.wait_list;
								that.roomPosition.version = socketDate.data.version;
								that.positionAllUserList = [...socketDate.data.stage_list, ...socketDate.data
									.wait_list
								];
								if (socketDate.data.step_down_user == that.userInfo.id) {
									// #ifdef APP
									that.trtcCloud.stopLocalAudio()
									// #endif
								}
								that.wheatBtnChange()
								break;
							case "task_ready_status":
								that.roomPosition.stage_list = socketDate.data.stage_list;
								socketDate.data.stage_list.forEach((item, index) => {
									if (item.ready_status != 1) {
										that.readyTimeFlag = true;
										that.times = 120;
										that.roomInterVal()
										return;
									}
								...
								break;
							case "fhl_key_word":
								that.keyWordData = socketDate.data
								break;
							case "fhl_task_begin":
								...
								that.readyTimeFlag = false;
								that.taskResultShow = false;
								that.readyInterVal = null;
								that.times = 30;
								that.competitionFlag = true;
								that.roomInterVal()
								that.silkText = '免费';
								that.$nextTick(() => {
							...
							case "fhl_task_trun":
								...
								clearInterval(that.interVal)
								that.times = 30;
								that.taskContentList = [];
								that.taskContentList.push(socketDate.data)
								that.taskData = socketDate.data;
								that.roomInterVal()
								// that.$nextTick(() => {
								// 	that.$refs.collision.hide()
								// })
								that.roomPosition.stage_list.forEach((item, index) => {
									if (item.user_id == Number(socketDate.data.answer_uid)) {
							...
							case "fhl_task_result":
								...
								that.tishiFlag = false;
								that.times = 120;
								that.competitionFlag = false;
								that.readyFlag = false;
								that.taskBeginTimes = 30;
								clearInterval(that.taskBeginInter)
								that.taskResultShow = true;
								that.taskResultData = socketDate.data;
								that.taskData = [];
								that.giftCurrentUserIndex = 0;
								that.silkText = '免费';
								break;
							case "room_trtc_status":
								if (socketDate.data.status == 0) {
									...
									// #ifdef APP
									that.addAudioRoom(that.userId, that.roomId)
									// #endif
								}
								break;
							case "close_room":
								that.$api("room.exitRoom", {
									room_id: that.roomData.room_id
								}).then(res => {
									if (res.code == 1) {
										console.log("返回")
									}
					...
					} else if (socketDate.cate == 'room_info_version') {
						if (that.roomId == socketDate.data.room_id) {
							if (socketDate.data.position != that.roomPosition.version) {
								that.getRoomPositionInfo()
							}
							if (socketDate.data.im != that.messageList.version) {
								that.getRoomImInfo()
							}
							if (socketDate.data.base != that.roomData.version) {
								that.getRoombaseInfo()
							}
							if (socketDate.data.task != that.taskVersion) {
								that.getRoomTaskInfo()
							}
						}
					} else if (socketDate.cate == "chat") {
						if (socketDate.data.room_id == that.roomId) {
							if (socketDate.data.message.type == "gift") {
								if (that.taskData.length == 0) {
									that.showSvga = true;
									that.$nextTick(() => {
										that.$refs.svgaPlayer.render(async (parser, player) => {
											let videoItem = await parser.load(socketDate.data
												.message
												.gift_svga);
											await player.setVideoItem(videoItem)
											player.loops = 1;
											player.setContentMode(socketDate.data.message
												.is_full_screen ==
												1 ?
												"AspectFill" :
												"AspectFit")
									...
									if (isShowGift == true) {
										...
										that.$nextTick(() => {
											that.$refs.svgaPlayer.render(async (parser, player) => {
												let videoItem = await parser.load(socketDate
													.data.message
													.gift_svga);
												await player.setVideoItem(videoItem)
												player.loops = 1;
							...
							that.messageList.list.push(socketDate.data);
							that.scrollBottom();
							that.$forceUpdate()
						}
					}
			...
			//退出房间
			exitRoom() {
				var isUser = this.roomPosition.stage_list.filter((item, index) => {
				...
				if (isUser.length == 0) {
					...
					if (this.competitionFlag == false) {
						...
						this.roomPosition.stage_list.forEach((item, index) => {
			...
			transport() {
				...
				this.roomPosition.stage_list.forEach((val, index) => {
					if (val.user_id == this.taskData.none_answer_id) {
						try {
			...
			//上麦
			addTalk() {
				for (var i = 0; i < this.roomPosition.wait_list.length; i++) {
					if (this.roomPosition.wait_list[i] == '') {
						this.add(1, i + 3)
						return;
					}
				}
				...
				this.roomPosition.stage_list.forEach((item, index) => {
					if (item.user_id == this.userInfo.id) {
						if (this.competitionFlag == false) {
							...
							this.showDrawer = false;
							this.exitRoomFlag = true;
							this.clickXiamaiFlag = true;
						}
						return;
					}
				})
				this.roomPosition.wait_list.forEach((item, index) => {
					if (item.user_id == this.userInfo.id) {
						this.add(2, item.key);
					}
				})
			...
			getKeyWode() {
				...
				try {
					that.$api("room.preCreateTask", {
						page: that.keyWordPage
					}).then(res => {
						if (res.code == 1) {
							that.keyWordList = [];
							that.keyWordList = res.data.list;
							that.keyWordShow = true;
							if (that.keyWordPage == res.data.page_data.total_page) {
								that.keyWordPage = 1;
							}
						} else {
							console.log("获取key失败")
			...
			keyWoridOk() {
				...
				try {
					if (this.roomPosition.stage_list.includes("") == false) {
						this.$api("room.taskReady", {
							room_id: this.roomId,
							key_word: this.keyWordList[this.keyWordIndex].word
						}).then(res => {
							this.keyWordIndex = 0;
							this.keyWordPage = 1;
						})
			...
			//答题倒计时
			taskBeginInterVal() {
				this.taskBeginTimes = 30;
				clearInterval(this.taskBeginInter)
				this.taskBeginInter = setInterval(() => {
					this.taskBeginTimes = --this.taskBeginTimes;
					if (this.taskBeginTimes <= 0) {
						clearInterval(this.taskBeginInter)
						this.submitFailTask()
					}
				}, 1000)
			...
			submitFailTask() {
				var that = this;
				console.log(this.taskData.answer_uid)
				that.roomPosition.stage_list.forEach((val, index) => {
					if (val.user_id == Number(this.taskData.answer_uid)) {
						if (Number(this.taskData.answer_uid) == this.userInfo.id) {
							try {
								that.$api("room.submitFailTask", {
									room_id: that.roomId,
									task_id: that.taskData.fhl_task_id,
			...
			//上下麦按钮切换
			wheatBtnChange() {
				var itemUser = this.positionAllUserList.filter((item, index) => {
					return item.user_id == this.userInfo.id
				})
				...
				this.readyFlag = this.roomPosition.stage_list.includes("")
				if (this.readyFlag) {
					clearInterval(this.interVal)
					this.tishiFlag = false;
					this.readyTimeFlag = false;
			...
			yuCreateRoom() {
				console.log(this.createroomData.is_real_auth)
				if (this.createroomData.is_real_auth == 0) {
					this.showDrawer = false;
					this.empower = true;
					return;
				}
				...
				if (this.roomType == 'scjl' && this.roomType == 'cyjl') {
					this.$api("room.createRoomOption", {
						type: this.createroomData.type[this.createRoomTypeIndex].key
					}).then(res => {
						console.log(res)
						if (res.code == 1) {
							this.createroomData = res.data;
			...
			createRoom() {
				...
				if (that.roomType == 'fhl') {
					...
					paremt = {
						...
						public_type: that.createroomData.public_type[that.createRoomIndex].key
					}
				} else {
					type = that.createroomData.type[that.createRoomTypeIndex].key
					paremt = {
						type: that.createroomData.type[that.createRoomTypeIndex].key,
						room_name: that.createRoomName,
						public_type: that.createroomData.public_type[that.createRoomIndex].key
				...
				try {
					that.$api("room.createRoom", paremt).then(res => {
						...
						if (res.code == 1) {
							...
							uni.setStorageSync("roomData", res.data)
							uni.redirectTo({
								url: `/pages/chat/chatRoom?roomId=${res.data.room_info.room_id}&type=${res.data.room_info.room_type}`,
								animationType: 'fade-in',
			...
			clickPractice() {
				var arr = [];
				arr = this.positionAllUserList.filter((item, index) => {
					return item.user_id == this.userInfo.id
				})
				if (arr.length == 0) {
			...
			sendGift() {
				var that = this;
				if (that.roomPosition.stage_list[0] == '' && that.roomPosition.stage_list[1] == '') {
					return;
				}
				if (that.giftCurrentIndex == -1) {
					uni.showToast({
						icon: "none",
						title: "请选择礼物"
					})
				...
				if (that.allGiftSelectFlag == true) {
					that.$api("gift.giveGift2", {
						room_id: that.roomData.room_id,
						to_user_id: [that.roomPosition.stage_list[0].user_id, that.roomPosition.stage_list[1]
							.user_id
						],
					...
					that.$api("gift.giveGift2", {
						room_id: that.roomData.room_id,
						to_user_id: [that.roomPosition.stage_list[that.giftCurrentUserIndex].user_id],
						gift_id: that.giftList[that.giftCurrentIndex].id
					}).then(res => {
						that.getUserInfo()
						if (res.code == 3001) {
			...
			addAudioRoom(userId, roomId) {
				const params = {
					...
					roomId: Number(roomId), // Please replace with your own room number 
