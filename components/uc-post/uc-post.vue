<template>
    ...
    <view class="flex mt-8">
        ...
        <view class="flex-1 mr-4">
            ...
            <view class="text-sm leading-none text-gray-400 mt-3">{{ $u.timeFrom(item.createtime, 'mm月dd日 hh:MM') }}</view>
            ...
            <view v-if="item.audio" @click="handlePlayAudio()" class="mt-4 flex items-center justify-center rounded w-32 h-12 bg-gradient-to-r from-pink-500 to-rose-400">
            ...
            <view class="flex mt-4">
                <view class="flex-1 flex items-center">
                    ...
                    <text class="text-sm leading-none text-gray-500 ml-1">{{ item.viewnums }}</text>
                </view>
                <view class="flex items-center">
                    ...
                    <text class="text-sm leading-none text-gray-500 ml-1">{{ item.diggnums }}</text>
                </view>
                <view class="flex items-center ml-4">
                    ...
                    <text class="text-sm leading-none text-gray-500 ml-1">{{ item.commentnums }}</text>
                </view>
            </view>
        </view>
        <view class="">
            <view v-if="item.ischat" class="border border-solid border-orange-500 p-2 rounded-full text-sm leading-none text-orange-500" @click="$u.route('/pages/user/home', { user_id: item.id })">私聊他</view>
            <view v-else>
